...
namespace StackExchange.Redis.Tests
{
    public class RealWorld : TestBase
    {
        ...
        {
            Log("first:");
            var config = ConfigurationOptions.Parse("localhost:6479,localhost:6480,name=Core (Q&A),tiebreaker=:RedisMaster,abortConnect=False");
            ...
            using (var conn = ConnectionMultiplexer.Connect("localhost:6479,localhost:6480,name=Core (Q&A),tiebreaker=:RedisMaster,abortConnect=False", Writer))
            {
                Log("");
                Log("pausing...");
