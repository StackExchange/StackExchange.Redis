...
internal static class RespReaderInternalExtensions
{
    extension(in RespReader reader)
    {
        ...
        internal RedisChannel ReadRedisChannel(RedisChannel.RedisChannelOptions options)
            => new(reader.ReadByteArray(), options);

        internal bool TryGetFirst(out string first)
        {
        ...
        internal RespPrefix GetFirstPrefix()
        {
    ...
    {
        internal bool SafeTryMoveNext() => reader.TryMoveNext(checkError: false) & !reader.IsError;

        internal void MovePastBof()
        {
            // if we're at BOF, read the first element, ignoring errors
            if (reader.Prefix is RespPrefix.None) reader.SafeTryMoveNext();
        }

        internal RedisValue[]? ReadPastRedisValues()
            => reader.ReadPastArray(static (ref r) => r.ReadRedisValue(), scalar: true);
    }

    internal static RespPrefix GetRespPrefix(ReadOnlySpan<byte> frame)
    {
    ...
    {
        internal ResultType ToResultType() => prefix switch
        {
