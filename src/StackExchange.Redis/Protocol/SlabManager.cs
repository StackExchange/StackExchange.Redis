...
internal sealed class SlabManager : IDisposable
{
    ...
    public IDisposable GetChunk(out Memory<byte> chunk)
    {
    ...
    public bool TryExpandChunk(IDisposable owner, ref Memory<byte> chunk)
        => owner is Slab slab && slab.TryExpand(ChunkSize, ref chunk);

    private IDisposable GetChunkSlow(out Memory<byte> chunk)
    {
        Slab? newSlab = null;
        do
        {
        ...
        [DoesNotReturn]
        static void ThrowDisposed() => throw new ObjectDisposedException(nameof(SlabManager));
    }
    ...
    internal sealed class Slab : IDisposable
    {
        ...
        public Slab(int size) => _array = ArrayPool<byte>.Shared.Rent(size);
        ...
        public void Dispose()
        {
            ...
            } while (Interlocked.CompareExchange(ref _offsetAndRefCount,
        ...
        public bool TryTake(int size, out Memory<byte> value)
        {
            ...
            while (true)
            {
                ...
                if (Interlocked.CompareExchange(ref _offsetAndRefCount,
                    (((long)(oldOffset + size)) << 32) | (long)(oldCount + 1), // update the offset and increment the count
                    oldOffsetAndRefCount) == oldOffsetAndRefCount)
                {
                    value = new Memory<byte>(_array, oldOffset, size);
                    return true;
                }
            }
            value = default;
        ...
        internal bool TryExpand(int size, ref Memory<byte> value)
        {
            if (size <= 0) ThrowInvalidSize();
            if (MemoryMarshal.TryGetArray<byte>(value, out var segment) && ReferenceEquals(segment.Array, _array))
            {
                ...
                while (true)
                {
                    ...
                    if (Interlocked.CompareExchange(ref _offsetAndRefCount,
                        ...
                        oldOffsetAndRefCount) == oldOffsetAndRefCount)
                    {
                        value = new Memory<byte>(_array, segment.Offset, segment.Count + size);
                        return true;
                    }
                }
            }
