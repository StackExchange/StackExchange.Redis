...
namespace StackExchange.Redis
{
    internal class RedisTransaction : RedisDatabase, ITransaction
    {
        ...
        private class TransactionMessage : Message, IMultiMessage
        {
            ...
            internal override void SetExceptionAndComplete(Exception exception, PhysicalBridge bridge, CommandFailureReason reason)
            {
                ...
                if (inner != null)
                {
                    ...
                    {
                        inner[i]?.Wrapped?.SetExceptionAndComplete(exception, bridge, reason);
                    }
                }
                base.SetExceptionAndComplete(exception, bridge, reason);
            }
