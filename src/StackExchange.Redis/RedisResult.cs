...
namespace StackExchange.Redis
{
    ...
    public abstract class RedisResult
    {
        ...
        /// </summary>
        internal static bool TryCreate(ITransportState transport, in RawResult result, [NotNullWhen(true)] out RedisResult? redisResult)
        {
            try
            {
                switch (result.Type)
                {
                    ...
                    case ResultType.MultiBulk:
                        ...
                        while (iter.MoveNext())
                        {
                            if (TryCreate(transport, in iter.Current, out var next))
                            {
                                arr[i++] = next;
                            }
                            else
