...
[full docs](/GeoRedundantFailover).
...
without the usual backwards-compatibility guarantees. That is the whole reason it is here. Geo-redundant failover is a
large, brand-new public API arriving in a single release, and once a surface that size is locked down it is locked down
