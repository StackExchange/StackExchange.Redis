...
- `toys/` — runnable samples and an in-process RESP server (`toys/StackExchange.Redis.Server`, used by tests as a managed fake server), Kestrel-hosted server, console tools. Its assembly and package keep the name `StackExchange.Redis.Server`, but its **namespace** is `StackExchange.Redis.ManagedServer`: a namespace and a type cannot both be reachable as `StackExchange.Redis.Server` (CS0435), and the client library uses the type for the server-scoped command group.
